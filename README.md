...
✅ **v0.6.0**: 模拟盘运行中 — 全链路（数据采集 → 因子 → 模型 → 回测 → 推理 → 执行）已打通，模拟盘脚本每日自动开/平仓，风控与审计日志完整。13 因子/39 特征，双模型 (V1/V2)。下一阶段：模拟盘监控与偏差分析。
...
- [x] 数据采集 + 因子计算 — v0.1.0 ✅
- [x] 工程化重构（配置中心、Backend 协议、IO 层拆分）— v0.2.0 ✅
...
- [x] 实盘就绪评估（结论：研究原型，不可实盘）— v0.5.0
- [x] 回测有效性验证（Rolling OOS 交叉验证 / Permutation Test / 随机基线）— v0.5.0
- [x] 13 因子扩展 + 模型 Registry + 安全杠杆计算 — v0.5.0
- [x] 执行层搭建（Broker / RiskManager / OrderManager）— v0.5.0
- [x] 风控与仓位管理（止损/杠杆限制/每日交易次数上限）— v0.5.0
- [x] 模拟盘脚本（run_simulation.py）— v0.6.0 ✅
- [ ] 模拟盘监控（摘要、偏差分析、报告）— Sprint 2
- [ ] 实盘（极小资金起步）
